...
// Step 1: Setup reports
console.log('Step 1: Setting up bundle reports...');
require('./setup-bundle-reports.js');

// Step 2: Generate reports
console.log('Step 2: Generating bundle reports...');
require('./ci-report-generator.js');

// Step 3: Serve reports
console.log('Step 3: Starting bundle report server...');
  ...
  <script>
    ...
    async function loadReports() {
      try {
        ...
        if (reports.length === 0) {
          reportsList.innerHTML = '<div class="no-reports">No reports available.</div>';
          return;
        }
        ...
        reportsList.innerHTML = reports.map(report => \`
          <li>
            ...
            <div class="report-date">Generated: \${new Date(report.date).toLocaleString()}</div>
            <div class="report-size">Size: \${(report.size / 1024).toFixed(2)} KB</div>
            <a href="/report/\${report.name}" class="btn">View Report</a>
          </li>
        \`).join('');
      } catch (error) {
...
// Start the server
server.listen(PORT, () => {
  console.log(`Bundle report server running at http://localhost:${PORT}/`);
